...
async function addUser(email: string, name: string, password: string, role: Role,) {
  ...
  async function addDirectory(name: string, path: string, parentId: number, ownerId: number, permissions: PermissionType[]) {
    try {
      const newDir = await prisma.directory.create({
        data: {name,path,parentId,ownerId},
      });
      const permission = await Promise.all(permissions.map(async (permissionType) => {
        return prisma.permission.create({
          data: {type: permissionType, directory: {
  ...
  // creating a file
  async function addFile(name: string, path: string, parentId: number, ownerId: number, content: string, permissions: PermissionType[]) {
    try {
...
export {addUser, addDirectory, addFile, readFile, removeUser, removeFile, removeDirectory, deleteAllData}